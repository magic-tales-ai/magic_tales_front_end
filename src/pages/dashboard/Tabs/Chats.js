...
//actions
import { activeTale, deleteStory } from "../../../redux/actions";
...
const Chats = ({ recentChatList, active_tale, ...props } = {}) => {
    ...
    const closeMenu = () => {
        ...
        if (taleChat) {
            taleChat[0]?.classList.remove("main-menu-show");
        }
    }
    ...
    const menuTales =
        <div className="d-flex p-3">
            ...
            <Link to='/profiles' size="xs" className="btn btn-outline-primary mx-2" >
                <span className="custom-icon"><img src={iconGroup} alt="icon group" /></span>
            </Link>

            <Link to='/library' size="xs" className="btn btn-outline-primary">
                <span className="custom-icon"><img src={iconLibrary} alt="icon library" /></span>
            </Link>
        </div>


    const openTaleChat = (e, chat) => {
        e.preventDefault();

        return;

        // set activeTale
        dispatch(activeTale(chat.uid));
        ...
        var unread = document.getElementById("unRead" + chat.uid);
        if (unread) {
        ...
        <React.Fragment>
            ...
            <SimpleBar className="chat-message-list" ref={ref}>
                <ul className="list-unstyled chat-list chat-user-list px-3" id="chat-list">
                    {
                        recentChatList.length > 0 && Object.entries(recentChatList).map(([key, chat]) => {
                            return (
                                <li key={chat.id} id={"conversation" + chat.id} className={chat.unRead ? "unread" : chat.isTyping ? "typing" : chat.id === active_tale ? "active" : ""}>
                                    <Link to="#" onClick={(e) => openTaleChat(e, chat)}>
                                        <div className="d-flex align-items-center">
                                            {
                                                !chat.profilePicture ?
                                                    <div className={"chat-user-img align-self-center me-1 ms-0"}>
                                                        <div className="avatar-xs">
                                                            <img src={imgDefault} className="rounded avatar-xs" alt={chat.title} />
                                                        </div>
                                                    </div>
                                                    :
                                                    <div className={"chat-user-img align-self-center me-1 ms-0"}>
                                                        <div className="avatar-xs">
                                                            <img src={chat.profilePicture} className="rounded avatar-xs" alt={chat.title} />
                                                        </div>
                                                    </div>
                                            }

                                            <div className="flex-grow-1 overflow-hidden">
                                                <h5 className="text-truncate font-size-14 mb-1 ms-1">{chat.title}</h5>
                                            ...
                                            <div className="flex-lg-column">
                                                <Dropdown isOpen={dropdownOpen === chat.id} toggle={(e) => toggle(e, chat.id)}>
                                                    <DropdownToggle
                                                        tag="a"
                                                    ...
                                                    <DropdownMenu className="dropdown-menu-end">
                                                        <DropdownItem onClick={() => { sendMessage({ command: websocket_commands_messages.SPIN_OFF, story_id: chat.id }) }}>{t("Create spin - off")}</DropdownItem>
...
export default connect(mapStateToProps, { activeTale })(Chats);