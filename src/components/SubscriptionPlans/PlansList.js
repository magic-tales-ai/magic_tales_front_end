...
const PlansListComponent = (props) => {
  ...
  return (
    <div
      id="plansList"
      className="w-100 d-flex flex-column align-items-center justify-content-center planes-box bg-black"
    >
      <div className="container py-5 text-center">
        <div className="title-plan mx-auto mb-4">
          <h1 className="ff-special fw-normal mb-4">
            {t("Choose Your Ideal Plan")}
          </h1>
          <p className="opacity-75 font-size-16">
            {t("Flexibility and Value: Our subscription plans have it all.")}
          </p>
        </div>
        ...
        <div className="swithces">
          <div className="form-check form-switch mx-auto">
            <Input
              ...
              id="pay"
            />
            <Label check className="form-check-label" for="pay">
              <span>{t("Pay monthly")}</span> <span>{t("Pay yearly")}</span>
            </Label>
          </div>
        </div>
      </div>
          ...
          return (
            <React.Fragment key={`plan-${p.get("id")}`}>
              <Plan
                plan={p}
                pay={pay}
                currentPlan={user && p.get("id") === user.get("plan").get("id")}
              />
            </React.Fragment>
          );
        })}
...
export const PlansList = connect(mapStateToProps, { loadPlansList })(
  PlansListComponent,
);
